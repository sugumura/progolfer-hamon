...
(function(process) {
  ...
  module.exports = function(grunt) {
    grunt.initConfig({
      ...
      uglify: {
        options: {
          ...
          mangle: {except: getExclusions()}
        },
        build: {files: {'output/<%= pkg.name.toLowerCase() %>-<%= version %>.min.js': getConfigValue('source')}}
      },
      concat: {
        options: {
          separator: '',
        ...
        build: {files: {'output/<%= pkg.name.toLowerCase() %>-<%= version %>.combined.js': getCombinedSource()}}
      },
      yuidoc: {compile: {
          name: '<%= pkg.name %>',
          version: '<%= version %>',
      ...
      clean: {docs: {src: ["<%= docsFolder %>/assets/scss"]}},
      copy: {
        docsZip: {files: [{
            expand: true,
            cwd: 'output/',
      ...
      updateversion: {preload: {
          file: '../src/preloadjs/version.js',
          version: '<%= version %>'
        }},
      clearversion: {preload: {file: '../src/preloadjs/version.js'}},
      sass: {docs: {
          options: {
            style: 'compressed',
            sourcemap: "none"
          },
          files: {'createjsTheme/assets/css/main.css': 'createjsTheme/assets/scss/main.scss'}
        }}
    });
    ...
    grunt.registerTask('docs', ["sass", "setDocsBase", "yuidoc", "resetBase", "clean:docs", "compress", "copy:docsZip"]);
    ...
    grunt.registerTask('nextlib', ["updateversion", "combine", "uglify", "clearversion", "copy:src"]);
    ...
    grunt.registerTask('coreBuild', ["updateversion", "combine", "uglify", "clearversion", "docs", "copy:src"]);
    grunt.registerTask('combine', 'Combine all source into a single, un-minified file.', ["concat"]);
  };
})(require('process'));
